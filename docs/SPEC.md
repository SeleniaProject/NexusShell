...
- **POSIX / GNU Coreutils**: `ls`, `grep`, `awk`, `sed`, など 250+ コマンド。
- **Bash 拡張**: 配列・連想配列、Here String (`<<<`)、プロセス置換、算術展開ほか。
- **PowerShell 互換 (オプション)**: *デフォルトでは無効*。`Get-Process` → `ps`, `Get-ChildItem` → `ls` など Bash 風ショートコマンドを第一級とし、PowerShell 形式は `enable-ps-aliases` 設定時のみ利用可能。
- **BusyBox モード**: `nxsh --busybox` で単一バイナリ軽量起動 (< 1 MiB)。
- **Z-shell 特化機能**: グロブ修飾子、リスト展開、パス補完オプション全対応。

### 4.2 スクリプト言語拡張

| 機能 | 例 | 説明 |
|------|----|------|
| パターンマッチ | `match $var { 1 => echo one, _ => echo other }` | Rust ライク構文 |
| 名前空間 | `use net::*` | クレート風インポート |
| クロージャ | `map { |x| $x * 2 }` | 高階関数サポート |
| ジェネリクス | `fn push<T>(list: mut [T], item: T)` | 型安全 API |
| エラーハンドリング | `try { cmd } catch { |e| warn $e }` | Result 型ベース |
| マクロ | `macro_rules! upper { ($x:expr) => { echo $x | tr a-z A-Z } }` | コンパイル時展開 |
...
- **リアルタイム構文解析**: 入力中にカラーリング・エラー表示。
- **インテリセンス補完**: 型とオプション情報から候補を推論。
- **ドキュメントポップアップ**: `F1` でカーソル直下コマンドの詳細を TUI オーバーレイ表示。
- **セッション録画 / 再生**: `rec start`, `rec stop`, `rec play` で手順共有。

### 4.5 プラグインシステム

| 形式 | 言語 | 隔離方式 | 例 |
|------|------|----------|-----|
| Native Crate | Rust | 同一プロセス / Capability 限定 | `cargo nxsh-plugin new foo` |
| WASM | Rust / C / Zig ほか | WASI Sandbox | `module load foo.wasm` |
| External Binary | 任意 | fork-exec | `foo.sh`, `foo.exe` |
