...
impl From<SerializableStyle> for ContentStyle {
    fn from(val: SerializableStyle) -> Self {
        ...
        if let Some(fg) = val.foreground {
            let target_color = if fg.contains("Blue") {
                Color::Blue
            } else if fg.contains("Red") {
                Color::Red
            } else if fg.contains("Green") {
                Color::Green
...
fn default_status_inactive() -> String {
    "#a4b0be".to_string()
}
fn default_completion_bg() -> String {
    "#2f3542".to_string()
}
fn default_completion_fg() -> String {
    "#dfe4ea".to_string()
}
fn default_completion_selected() -> String {
    "#00f5ff".to_string()
}
...
impl NexusTheme {
    ...
    pub fn load_from_file(path: &PathBuf) -> Result<Self> {
        ...
        if let Some(ext) = path.extension() {
            match ext.to_str().unwrap_or("") {
                "json" => {
                    serde_json::from_str(&content).with_context(|| "Failed to parse JSON theme")
                }
                "toml" => toml::from_str(&content).with_context(|| "Failed to parse TOML theme"),
    ...
    pub fn save_to_file(&self, path: &PathBuf, format: ThemeFormat) -> Result<()> {
        ...
        fs::write(path, content).with_context(|| format!("Failed to write theme file: {path:?}"))
    }
...
impl ThemeManager {
    ...
    pub fn new() -> Result<Self> {
        let theme_directory =
            Self::get_theme_directory().unwrap_or_else(|_| PathBuf::from("themes"));
    ...
    pub fn discover_themes(&self) -> Result<()> {
        ...
        if let Ok(entries) = std::fs::read_dir(&self.theme_directory) {
            let mut themes = match self.available_themes.write() {
                Ok(g) => g,
    ...
    pub fn get_theme(&self, name: &str) -> Result<NexusTheme> {
        let themes = match self.available_themes.read() {
            Ok(g) => g,
            Err(poisoned) => poisoned.into_inner(),
        };
        themes
    ...
    pub fn set_theme(&self, theme_name: &str) -> Result<()> {
        ...
        if let Some(theme) = themes.get(theme_name) {
            let mut current = match self.current_theme.write() {
                Ok(g) => g,
                Err(poisoned) => poisoned.into_inner(),
            };
            *current = theme.clone();
    ...
    pub fn list_themes(&self) -> Vec<String> {
        match self.available_themes.read() {
...
pub fn list_theme_files() -> Result<Vec<PathBuf>> {
    let theme_dir = ThemeManager::get_theme_directory().unwrap_or_else(|_| PathBuf::from("themes"));

    let mut files = Vec::new();
    if theme_dir.exists() {
        for entry in fs::read_dir(&theme_dir)
            .with_context(|| format!("Failed to read theme directory: {:?}", theme_dir))?
        {
            let entry = entry?;
            let path = entry.path();
            if let Some(ext) = path.extension() {
