...
fn main() -> anyhow::Result<()> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            "--dir" => {
                themes_dir = Some(PathBuf::from(args.next().expect("--dir requires a path")))
            }
            "--out-format" => {
                out_format = Some(args.next().expect("--out-format requires md|csv|json"))
            }
            "--out" => out_path = Some(PathBuf::from(args.next().expect("--out requires a path"))),
            "--min-contrast" => {
                _min_contrast = Some(
                    args.next()
                        .expect("--min-contrast requires a number")
                        .parse::<f64>()
                        .unwrap_or(4.5),
                )
            }
    ...
    if strict && invalid_themes > 0 {
        anyhow::bail!("{} themes invalid", invalid_themes);
    }
    Ok(())
}
