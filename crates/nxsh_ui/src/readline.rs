...
impl ReadLine {
    ...
    fn apply_completion(&mut self, completion: &CompletionResult) -> io::Result<()> {
        let (_prefix, word_start, word) = self
            .completion_engine
            .parse_completion_context(&self.line, self.cursor_pos);
        let mut replacement = completion.completion.clone();

        // Quote paths with spaces or special chars unless already quoted
        if matches!(
            completion.completion_type,
            crate::completion::CompletionType::File | crate::completion::CompletionType::Directory
        ) {
            let needs_quote = replacement.chars().any(|c| {
                matches!(
                    c,
                    ' ' | '\t' | '(' | ')' | '[' | ']' | '{' | '}' | '&' | ';' | '"' | '\''
                )
            ...
            if r == 0 {
                ...
                for c in 0..cols {
                    ...
                    if idx < names.len() {
                        ...
                        let label = &names[idx];
                        let padding =
                            col_width.saturating_sub(UnicodeWidthStr::width(label.as_str()));
                        if selected {
...
impl NexusCompleter {
    pub fn parse_completion_context<'a>(
        &self,
        input: &'a str,
        cursor_pos: usize,
    ) -> (&'a str, usize, &'a str) {
