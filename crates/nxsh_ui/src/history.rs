...
impl History {
    ...
    pub fn add_entry(&mut self, command: String) {
        ...
        let entry = HistoryEntry::new(command.trim().to_string())
    ...
    pub fn previous(&mut self) -> Option<String> {
        ...
        self.entries.get(new_index).map(|entry| entry.command.clone())
    }
        ...
        match self.current_index {
            ...
            Some(index) => {
                if index + 1 < self.entries.len() {
                    ...
                    self.entries.get(index + 1).map(|entry| entry.command.clone())
                } else {
                    self.current_index = None;
                    None
                }
    ...
    pub fn stats(&self) -> HistoryStats {
        ...
        let unique_commands = self.entries
            .iter()
        ...
        let commands_by_frequency = {
            ...
            freq_vec.into_iter().take(10).map(|(k, v)| (k.clone(), v)).collect()
        };
