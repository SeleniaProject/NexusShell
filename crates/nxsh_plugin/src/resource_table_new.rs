...
/// Concrete resource entry implementation
#[derive(Debug)]
pub struct ConcreteResourceEntry<T: Send + Sync + 'static> {
    pub info: ResourceInfo,
    /// Resource is held behind Arc to allow safe shared borrowing out of the table
    pub resource: Arc<T>,
}
...
impl<T: Send + Sync + 'static + std::fmt::Debug> ResourceEntry for ConcreteResourceEntry<T> {
    ...
    fn as_any(&self) -> &dyn Any {
        // Expose the Arc<T> for downcasting and cloning by callers
        &self.resource
    }
    ...
    fn can_drop(&self) -> bool {
        matches!(self.info.state, ResourceState::Active | ResourceState::Idle)
    }
    
    fn prepare_cleanup(&mut self) -> Result<()> {
        self.info.state = ResourceState::MarkedForCleanup;
        Ok(())
    }
}
...
impl ResourceTable {
    ...
    pub async fn add_resource<T: Send + Sync + 'static + std::fmt::Debug>(
        ...
        info!("Resource {} ({}) created for plugin {}", resource_id, resource_type, plugin_id);
        Ok(resource_id)
    }

    /// Get a resource from the table
    pub async fn get_resource<T: 'static>(&self, resource_id: &ResourceId) -> Result<Option<Arc<T>>> {
        let mut resources = self.resources.write().await;
        ...
        if let Some(entry) = resources.get_mut(resource_id) {
            ...
            // Downcast to Arc<T> and clone to return a new strong reference
            if let Some(arc_any) = entry.as_any().downcast_ref::<Arc<T>>() {
                debug!("Resource {} accessed successfully", resource_id);
    ...
    pub async fn cleanup_idle_resources(&self) -> Result<usize> {
        ...
        info!("Cleaned up {} idle resources", cleanup_count);
        Ok(cleanup_count)
    }
    ...
    pub async fn cleanup_plugin_resources(&self, plugin_id: &str) -> Result<usize> {
        ...
        info!("Cleaned up {} resources for plugin {}", cleanup_count, plugin_id);
        Ok(cleanup_count)
    }
}
