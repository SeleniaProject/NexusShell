...
impl SignatureVerifier {
    ...
    pub async fn verify_plugin<P: AsRef<Path>>(
        ...
        let tuf_entry = self
            .tuf_metadata
            .find_plugin(&metadata.name, &metadata.version)
            .ok_or_else(|| {
                PluginError::SecurityError(format!(
                    "Plugin '{}' v{} not found in TUF metadata",
        ...
        let signature_data = tokio::fs::read(&signature_path).await.map_err(|e| {
            PluginError::SecurityError(format!("Failed to read signature file: {e}"))
        })?;
    ...
    pub async fn sign_plugin<P: AsRef<Path>>(
        ...
        let payload_bytes =
            serde_json::to_vec(&payload).context("Failed to serialize signature payload")?;
        ...
        tokio::fs::write(&signature_path, signature_json)
            .await
    ...
    pub async fn add_trusted_key(
        ...
        self.trusted_keys
            .insert(key_id.clone(), public_key.to_base64());
    ...
    pub async fn update_tuf_metadata(&mut self, metadata: TufMetadata) -> Result<()> {
        ...
        info!(
            "Updated TUF metadata to version {}",
            self.tuf_metadata.version
        );
        Ok(())
    }
    ...
    async fn load_trusted_keys(&mut self) -> Result<()> {
        ...
        if keys_path.exists() {
            let keys_data = tokio::fs::read_to_string(&keys_path)
                .await
    ...
    async fn save_trusted_keys(&self) -> Result<()> {
        ...
        let keys_json =
            serde_json::to_string_pretty(&keys_file).context("Failed to serialize trusted keys")?;

        tokio::fs::write(&keys_path, keys_json)
            .await
    ...
    async fn load_tuf_metadata(&mut self) -> Result<()> {
        ...
        if metadata_path.exists() {
            let metadata_data = tokio::fs::read_to_string(&metadata_path)
                .await
    ...
    async fn verify_tuf_metadata(&self) -> Result<()> {
        ...
        if !self
            .verify_tuf_metadata_signature(&self.tuf_metadata)
            .await?
        {
    ...
    async fn is_key_revoked(&self, key_id: &str) -> Result<bool> {
        ...
        Ok(self
            .key_rotation_log
            .iter()
            .any(|entry| entry.key_id == key_id && entry.action == KeyAction::Revoked))
    }
