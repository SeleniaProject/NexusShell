...
impl SecuritySandbox {
    ...
    pub async fn create_policy(&self, plugin_id: &str, config: PolicyConfig) -> Result<SecurityPolicy> {
        ...
        // Log policy creation
        self.audit_logger.log_policy_creation(plugin_id, &policy).await;
    ...
    pub async fn validate_operation(&self, plugin_id: &str, operation: &SecurityOperation) -> Result<bool> {
        ...
        if let Some(expires_at) = policy.expires_at {
            if SystemTime::now() > expires_at {
                self.record_violation(plugin_id, SecurityViolationType::ExpiredPolicy, "Policy has expired").await;
                return Ok(false);
            }
        }
        ...
        let is_allowed = match operation {
            SecurityOperation::FileAccess { path, operation_type } => {
                self.validate_file_access(policy, path, operation_type).await?
            },
            ...
            SecurityOperation::ResourceAllocation { resource_type, amount } => {
                self.validate_resource_allocation(policy, resource_type, *amount).await?
            },
        };

        // Log operation attempt
        self.audit_logger.log_operation_attempt(plugin_id, operation, is_allowed).await;

        // Update threat detection
        if !is_allowed {
            self.threat_detector.record_suspicious_activity(plugin_id, operation).await;
        }
    ...
    pub async fn apply_sandbox_restrictions(&self, plugin_id: &str, process_id: u32) -> Result<()> {
        ...
        // Apply memory limits
        self.apply_memory_limits(process_id, policy.max_memory).await?;

        // Apply CPU limits
        self.apply_cpu_limits(process_id, policy.max_cpu_time).await?;

        // Apply file descriptor limits
        self.apply_fd_limits(process_id, policy.max_file_handles).await?;

        // Apply capability restrictions
        self.apply_capability_restrictions(process_id, &policy.capabilities).await?;
    ...
    async fn validate_file_access(&self, policy: &SecurityPolicy, path: &Path, operation_type: &FileOperationType) -> Result<bool> {
        // Check if path is in allowed paths
        let path_allowed = policy.allowed_paths.iter().any(|allowed_path| {
            path.starts_with(allowed_path)
        });

        if !path_allowed {
            self.record_violation(&policy.plugin_id, SecurityViolationType::UnauthorizedFileAccess, 
                &format!("Attempted to access unauthorized path: {path:?}")).await;
            return Ok(false);
        }
        ...
        match operation_type {
            ...
            FileOperationType::Execute => Ok(policy.capabilities.contains(&Capability::FileExecute)),
    ...
    async fn validate_network_access(&self, policy: &SecurityPolicy, host: &str, port: u16) -> Result<bool> {
        ...
        if let Some(allowed_hosts) = &policy.allowed_network_hosts {
            if !allowed_hosts.contains(&host.to_string()) {
                self.record_violation(&policy.plugin_id, SecurityViolationType::UnauthorizedNetworkAccess,
                    &format!("Attempted to access unauthorized host: {host}")).await;
                return Ok(false);
            }
        }
    ...
    async fn validate_syscall(&self, policy: &SecurityPolicy, syscall_name: &str) -> Result<bool> {
        if let Some(allowed_syscalls) = &policy.allowed_syscalls {
            if !allowed_syscalls.contains(&syscall_name.to_string()) {
                self.record_violation(&policy.plugin_id, SecurityViolationType::UnauthorizedSyscall,
                    &format!("Attempted unauthorized syscall: {syscall_name}")).await;
                return Ok(false);
            }
        }
    ...
    async fn validate_resource_allocation(&self, policy: &SecurityPolicy, resource_type: &ResourceType, amount: u64) -> Result<bool> {
        match resource_type {
            ResourceType::Memory => {
                let current_usage = self.resource_monitor.get_memory_usage(&policy.plugin_id).await.unwrap_or(0);
                if current_usage + amount > policy.max_memory {
                    self.record_violation(&policy.plugin_id, SecurityViolationType::ResourceLimitExceeded,
                        &format!("Memory allocation would exceed limit: {} + {} > {}", current_usage, amount, policy.max_memory)).await;
                    return Ok(false);
                }
            },
            ResourceType::FileHandles => {
                let current_handles = self.resource_monitor.get_file_handle_count(&policy.plugin_id).await.unwrap_or(0);
                    if current_handles + amount > policy.max_file_handles {
                        self.record_violation(&policy.plugin_id, SecurityViolationType::ResourceLimitExceeded, "File handle allocation would exceed limit").await;
                    return Ok(false);
                }
            },
        }
    ...
    async fn record_violation(&self, plugin_id: &str, violation_type: SecurityViolationType, description: &str) {
        ...
        // Log violation
        warn!("Security violation in plugin {}: {:?} - {}", plugin_id, &violation.violation_type, description);
    ...
    async fn apply_memory_limits(&self, process_id: u32, max_memory: u64) -> Result<()> {
        ...
        #[cfg(unix)]
        {
            ...
            debug!("Applied memory limit {} to process {}", max_memory, process_id);
        }
    ...
    /// Apply capability restrictions
    async fn apply_capability_restrictions(&self, process_id: u32, capabilities: &[Capability]) -> Result<()> {
...
impl AuditLogger {
    ...
    async fn log_operation_attempt(&self, plugin_id: &str, operation: &SecurityOperation, allowed: bool) {
        self.log_entry(AuditLogEntry {
            plugin_id: plugin_id.to_string(),
            event_type: if allowed { AuditEventType::OperationAllowed } else { AuditEventType::OperationDenied },
...
pub enum SecurityOperation {
    FileAccess { path: PathBuf, operation_type: FileOperationType },
    NetworkAccess { host: String, port: u16 },
    SystemCall { syscall_name: String },
    ResourceAllocation { resource_type: ResourceType, amount: u64 },
}
...
impl std::fmt::Display for SecurityViolationType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SecurityViolationType::UnauthorizedNetworkAccess => write!(f, "Unauthorized Network Access"),
...
mod tests {
    ...
    async fn test_file_access_validation() {
        ...
        let allowed = sandbox.validate_operation("test_plugin", &operation).await.unwrap();
        assert!(allowed);
    }
}
