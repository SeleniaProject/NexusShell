    ...
    pub fn check_env_access(&self, var_name: &str, write: bool) -> Result<()> {
        if write {
            if self.environment.write_vars.contains(var_name) {
                ...
                Err(anyhow!(
                    "Write access to environment variable '{}' denied",
                    var_name
                ))
            }
            ...
            Err(anyhow!(
                "Read access to environment variable '{}' denied",
                var_name
            ))
        }
    }
...
impl PermissionManager {
    ...
    pub async fn create_execution_context(
        ...
        let permission_set = self
            .create_minimal_permission_set(&validated_capabilities)
            .await?;

        // Create sandbox constraints
        let sandbox_constraints = self
            .create_sandbox_constraints(&policy, &permission_set)
            .await?;
            ...
            active.insert(
                ...
                ActivePermissionSet {
                    ...
                    expires_at: SystemTime::now()
                        + Duration::from_secs(policy.session_timeout_seconds),
                },
            );
        }
        ...
        info!(
            "Created execution context for plugin '{}' with {} capabilities",
            plugin_id,
            validated_capabilities.len()
        );
    ...
    pub async fn check_permission(
        ...
        let permission_set = active_permissions.get(plugin_id).ok_or_else(|| {
            PluginError::SecurityError(format!("No active permissions for plugin '{plugin_id}'"))
        })?;
        ...
        let result = self
            .evaluate_permission(&permission_set.context, operation)
            .await?;
    ...
    pub async fn revoke_permissions(&self, plugin_id: &str, reason: String) -> Result<()> {
        ...
        if active.remove(plugin_id).is_some() {
            // Log revocation
            self.log_permission_event(plugin_id, PermissionAction::Revoked, reason)
                .await;
    ...
    pub async fn create_capability_sandbox(
        ...
        let sandbox = CapabilitySandbox {
            ...
            network_access: self
                .create_network_sandbox(&context.sandbox_constraints)
                .await?,
            process_access: self
                .create_process_sandbox(&context.sandbox_constraints)
                .await?,
    ...
    async fn create_minimal_permission_set(
        ...
        for capability in capabilities {
            if let Some(def) = capability_defs.get(capability) {
                permission_set
                    .derived_permissions
                    .extend(def.required_permissions.clone());
            }
        }
    ...
    async fn evaluate_permission(
        ...
        match operation {
            PermissionOperation::FileRead { path } => {
                ...
                {
                    if self
                        .is_path_allowed(path, &context.sandbox_constraints.file_system)
                        .await?
                    {
                    ...
                    Ok(PermissionResult::denied(
                        "File read capability not granted".to_string(),
                    ))
                }
            }
            PermissionOperation::FileWrite { path } => {
                    ...
                    .contains(&"file_write".to_string())
                {
                        ...
                        .await?
                    {
                        ...
                        Ok(PermissionResult::denied(
                            "Path write access denied".to_string(),
                        ))
                    }
                } else {
                    Ok(PermissionResult::denied(
                        "File write capability not granted".to_string(),
                    ))
                }
            }
            PermissionOperation::NetworkRequest { url } => {
                ...
                {
                    if self
                        .is_url_allowed(url, &context.sandbox_constraints.network)
                        .await?
                    {
                        Ok(PermissionResult::allowed())
                    } else {
                    ...
                    Ok(PermissionResult::denied(
                        "Network capability not granted".to_string(),
                    ))
                }
            }
            PermissionOperation::ProcessExecute { command } => {
                if context
                    .permission_set
                    .capabilities
                    .contains(&"command_execute".to_string())
                {
                    if self
                        .is_command_allowed(command, &context.sandbox_constraints.process)
                        .await?
                    {
                        Ok(PermissionResult::allowed())
                    } else {
                        Ok(PermissionResult::denied(
                            "Command execution denied".to_string(),
                        ))
                    }
                } else {
                    Ok(PermissionResult::denied(
                        "Process execution capability not granted".to_string(),
                    ))
                }
            }
            PermissionOperation::EnvironmentRead { variable: _ } => {
                if context
                    .permission_set
                    .capabilities
                    .contains(&"env_read".to_string())
                {
                    Ok(PermissionResult::allowed())
                } else {
                    Ok(PermissionResult::denied(
                        "Environment read capability not granted".to_string(),
                    ))
                }
            }
        }
    ...
    async fn is_path_allowed(
        ...
        match restrictions {
            ...
            FileSystemRestrictions::ReadOnly { allowed_paths } => Ok(allowed_paths
                .iter()
                .any(|allowed| path.starts_with(allowed))),
            FileSystemRestrictions::Limited { allowed_paths, .. } => Ok(allowed_paths
                .iter()
                .any(|allowed| path.starts_with(allowed))),
    ...
    async fn is_url_allowed(&self, url: &str, restrictions: &NetworkRestrictions) -> Result<bool> {
        match restrictions {
    ...
    async fn create_fs_sandbox(
        &self,
        constraints: &SandboxConstraints,
    ) -> Result<FileSystemSandbox> {
    ...
    async fn create_network_sandbox(
        &self,
        constraints: &SandboxConstraints,
    ) -> Result<NetworkSandbox> {
        Ok(NetworkSandbox {
...
mod tests {
    ...
    async fn test_execution_context_creation() {
        ...
        let context = manager
            .create_execution_context("test-plugin", &metadata, &["file_read".to_string()])
            .await
            .unwrap();
    ...
    async fn test_permission_checking() {
        ...
        let result = manager
            .check_permission("test-plugin", &operation)
            .await
            .unwrap();
