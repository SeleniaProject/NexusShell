...
mod dependency_gating_tests {
    ...
    fn test_reqwest_completely_eliminated() {
        ...
        for features in feature_combinations {
            ...
            let output = cmd.output().expect("Failed to execute cargo tree command");

            let stdout =
                String::from_utf8(output.stdout).expect("Invalid UTF-8 in cargo tree output");
    ...
    fn test_ureq_minimal_configuration() {
        ...
        assert!(
            !ureq_lines.is_empty(),
            "ureq should be present when updates feature is enabled"
        );
    ...
    fn test_dependency_size_reduction() {
        ...
        eprintln!(
            "Full build dependencies: {full_count}, Minimal build dependencies: {minimal_count}"
        );
