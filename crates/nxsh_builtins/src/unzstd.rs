...
fn decompress_file(input_path: &Path, options: &UnzstdOptions) -> Result<()> {
    let input_data = if input_path.to_str() == Some("-") {
        ...
        if !input_path.exists() {
            return Err(anyhow!(
                "unzstd: {}: No such file or directory",
                input_path.display()
            ));
        }
    ...
    if options.stdout || input_path.to_str() == Some("-") {
        ...
        if output_path.exists() && !options.force {
            return Err(anyhow!("unzstd: {}: File exists", output_path.display()));
        }
        ...
        if options.verbose && !options.quiet {
            eprintln!("  {}: decompressed successfully", output_path.display());
        }
    }
...
fn decompress_zstd_data(data: &[u8]) -> Result<Vec<u8>> {
    #[cfg(feature = "compression-zstd")]
    {
        ...
        let mut decoder =
            StreamingDecoder::new(data).context("unzstd: failed to initialize zstd decoder")?;
