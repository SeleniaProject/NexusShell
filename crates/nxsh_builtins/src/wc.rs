...
pub fn wc_cli(args: &[String]) -> Result<()> {
        ...
        if arg.starts_with("--") {
            match arg.as_str() {
                ...
                "--files0-from" => {
                    idx += 1;
                    if idx >= args.len() {
                        return Err(anyhow!("wc: option '--files0-from' requires an argument"));
                    }
    ...
    let inputs: Vec<String> = {
        ...
        let mut pos_inputs: Vec<String> = if idx >= args.len() {
            Vec::new()
        } else {
            args[idx..].to_vec()
        };
        if list_inputs.is_empty() && pos_inputs.is_empty() {
            vec!["-".to_string()]
        } else {
...
fn accumulate(
    acc: &mut (usize, usize, usize, usize, usize),
    add: (usize, usize, usize, usize, usize),
) {
    acc.0 += add.0;
    acc.1 += add.1;
    acc.2 += add.2;
...
fn count_stream(path: &str, mode: Mode) -> Result<(usize, usize, usize, usize, usize)> {
    ...
    if mode.contains(Mode::LINES)
        || mode.contains(Mode::WORDS)
        || mode.contains(Mode::CHARS)
        || mode.contains(Mode::MAXLINE)
    {
...
mod tests {
    ...
    fn count_basic() {
        ...
        let counts = count_stream(
            &path,
            Mode::LINES | Mode::WORDS | Mode::BYTES | Mode::CHARS | Mode::MAXLINE,
        )
        .unwrap();
    ...
    fn files0_from_file_and_args_mix() {
        ...
        let args = vec!["--files0-from".to_string(), list_path, p2.clone()];
        // Should not error
        wc_cli(&args).unwrap();
    }
    ...
    fn test_single_line_no_newline() {
        ...
        let path = tmp.path().to_str().unwrap().to_string();
        let counts =
            count_stream(&path, Mode::LINES | Mode::WORDS | Mode::BYTES | Mode::CHARS).unwrap();
