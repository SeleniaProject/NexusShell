...
use std::fs::File;
use std::path::Path;
use lzma_rs::{xz_compress, xz_decompress, lzma_compress, lzma_decompress};
...
fn decompress_stream<R: Read, W: Write>(
    ...
    let decompressed_data = match options.format {
        CompressionFormat::Xz => {
            // Try XZ format first
            match xz_decompress(&compressed_data) {
                Ok(data) => data,
                Err(_) => {
                    // Fallback to LZMA if XZ fails
                    lzma_decompress(&compressed_data)
        ...
        CompressionFormat::Auto => {
            ...
            if let Ok(data) = xz_decompress(&compressed_data) {
                data
            } else if let Ok(data) = lzma_decompress(&compressed_data) {
                data
            } else {
...
fn determine_compressed_filename(input: &str, format: &CompressionFormat) -> String {
    match format {
        CompressionFormat::Xz | CompressionFormat::Auto => format!("{}.xz", input),
        CompressionFormat::Lzma => format!("{}.lzma", input),
        CompressionFormat::Raw => format!("{}.lz", input),
    }
}
...
fn get_xz_file_info(filename: &str) -> Result<XzFileInfo> {
    ...
    let uncompressed_data = xz_decompress(&compressed_data)
