...
use crate::ui_design::{
    Colorize, ItemStatus, StatusItem, StatusDashboard, DashboardSection, SectionStyle,
    CommandWizard, WizardStep, InputType
};
...
impl SmartAliasManager {
    ...
    fn initialize_default_aliases(&mut self) {
        let default_aliases = vec![
            // File operations - ultra short
            ("l", "ls", "List files", AliasCategory::FileManagement),
            ("ll", "ls -la", "Long list with hidden files", AliasCategory::FileManagement),
            ("la", "ls -la", "List all files", AliasCategory::FileManagement),
            ("lt", "ls -lt", "List by time", AliasCategory::FileManagement),
            ("lh", "ls -lh", "Human readable sizes", AliasCategory::FileManagement),
            ("tree", "ls -R", "Recursive list", AliasCategory::FileManagement),
            
            // Navigation - minimal keystrokes
            (".", "pwd", "Current directory", AliasCategory::Navigation),
            ("..", "cd ..", "Go up one level", AliasCategory::Navigation),
            ("...", "cd ../..", "Go up two levels", AliasCategory::Navigation),
            ("....", "cd ../../..", "Go up three levels", AliasCategory::Navigation),
            ("~", "cd ~", "Go home", AliasCategory::Navigation),
            ("-", "cd -", "Go to previous directory", AliasCategory::Navigation),
            ("root", "cd /", "Go to root", AliasCategory::Navigation),
            
            // Text processing shortcuts
            ("c", "cat", "Display file", AliasCategory::TextProcessing),
            ("h", "head", "Show first lines", AliasCategory::TextProcessing),
            ("t", "tail", "Show last lines", AliasCategory::TextProcessing),
            ("g", "grep --color=auto", "Colorized grep", AliasCategory::TextProcessing),
            ("gi", "grep -i --color=auto", "Case-insensitive grep", AliasCategory::TextProcessing),
            ("gr", "grep -r --color=auto", "Recursive grep", AliasCategory::TextProcessing),
            
            // System shortcuts
            ("cls", "clear", "Clear screen", AliasCategory::SystemAdmin),
            ("x", "exit", "Exit shell", AliasCategory::SystemAdmin),
            ("q", "exit", "Quick exit", AliasCategory::SystemAdmin),
            ("hist", "history", "Command history", AliasCategory::SystemAdmin),
            ("env", "printenv", "Show environment", AliasCategory::SystemAdmin),
            ("path", "echo $PATH", "Show PATH", AliasCategory::SystemAdmin),
            
            // File operations
            ("cp", "cp -i", "Safe copy with confirmation", AliasCategory::FileManagement),
            ("mv", "mv -i", "Safe move with confirmation", AliasCategory::FileManagement),
            ("rm", "rm -i", "Safe remove with confirmation", AliasCategory::FileManagement),
            ("mkdir", "mkdir -p", "Create directory path", AliasCategory::FileManagement),
            
            // Process management
            ("ps", "ps aux", "List all processes", AliasCategory::ProcessControl),
            ("psg", "ps aux | grep", "Find process", AliasCategory::ProcessControl),
            ("k", "kill", "Kill process", AliasCategory::ProcessControl),
            ("ka", "killall", "Kill all processes", AliasCategory::ProcessControl),
            
            // Network utilities
            ("ping", "ping -c 4", "Ping 4 times", AliasCategory::Network),
            ("ports", "netstat -tuln", "Show open ports", AliasCategory::Network),
            ("ip", "ip addr show", "Show IP addresses", AliasCategory::Network),
            
            // Development shortcuts
            ("py", "python3", "Python interpreter", AliasCategory::Development),
            ("node", "node", "Node.js", AliasCategory::Development),
            ("cargo", "cargo", "Rust cargo", AliasCategory::Development),
            ("git", "git", "Git version control", AliasCategory::Development),
            ("vim", "vim", "Vim editor", AliasCategory::Development),
            ("code", "code", "VS Code", AliasCategory::Development),
        ];
    ...
    pub fn save_aliases(&self) -> Result<()> {
        ...
        // This is a simplified implementation
        println!("{}", "Aliases saved successfully !");
        Ok(())
    }
    
    pub fn show_suggestions(&self) -> Result<()> {
        println!("{}", "Smart Alias Suggestions ");
        println!("{}", "-".repeat(60));
        ...
        for (i, suggestion) in self.suggestions.iter().enumerate() {
            ...
            println!("   Command: {}", suggestion.command_pattern);
            println!("   Reason: {}", suggestion.reason);
            println!("   Confidence: {:.0}%", (suggestion.confidence * 100.0));
        }
        
        println!("{}", "Use smart_alias create <name> <command> to create an alias ");
        Ok(())
    }
    ...
    pub fn run_interactive_wizard(&mut self) -> Result<()> {
        ...
        wizard.add_step(WizardStep {
            ...
            title: "What is the alias name?".to_string(),
...
fn import_aliases(manager: &mut SmartAliasManager, file_path: &str) -> Result<()> {
        ...
        match parse_alias_line(line) {
            Ok(Some((name, command, description))) => {
                ...
                if manager.aliases.contains_key(&name) {
                    println!("{}", format!("⚠�E�E�E�E Line {}: Alias '{}' already exists, skipping", 
                ...
                println!("{}", format!("✁EImported: {}", name).success());
            }
            Ok(None) => {
            ...
            Err(e) => {
                println!("{}", format!("❁ELine {}: Parse error: {}", line_num + 1, e).error());
                error_count += 1;
            }
        }
    }
    ...
    println!("{}", format!("  ✁EImported: {} aliases", imported_count).success());
    if skipped_count > 0 {
        println!("{}", format!("  ⚠�E�E�E�E Skipped:  {} duplicates", skipped_count).warning());
    }
    if error_count > 0 {
        println!("{}", format!("  ❁EErrors:   {} lines", error_count).error());
    }
