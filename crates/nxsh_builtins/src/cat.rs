...
use std::io::{self, BufRead, BufReader, BufWriter, Read, Write}; // BufRead 忁E��E(ジェネリチE��墁E�� / reader 生�Eで使用)
...
use encoding_rs::{Encoding, ISO_8859_2, UTF_16BE, UTF_16LE, UTF_8, WINDOWS_1252};
...
use memmap2::MmapOptions; // progress bars optional
...
impl IndicatifProgressBar {
    ...
    fn set_style(&self, _style: IndicatifProgressStyle) -> &Self {
        self
    }
...
impl IndicatifProgressStyle {
    ...
    fn template(self, _t: &str) -> Result<Self, ()> {
        Ok(Self)
    }
    fn progress_chars(self, _c: &str) -> Self {
        Self
    }
}
#[cfg(not(feature = "progress-ui"))]
...
impl IndicatifMultiProgress {
    ...
    fn add(&self, pb: IndicatifProgressBar) -> IndicatifProgressBar {
        pb
    }
}
use console::style;
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            ...
            "--threads" => {
                ...
                if i >= args.len() {
                    return Err(anyhow!(
                        t!("error-missing-argument", "option" => "--threads")
                    ));
                }
                options.max_threads = args[i]
                    .parse()
            ...
            "--encoding" => {
                ...
                if i >= args.len() {
                    return Err(anyhow!(
                        t!("error-missing-argument", "option" => "--encoding")
                    ));
                }
                options.encoding = match args[i].to_lowercase().as_str() {
                    ...
                    _ => {
                        return Err(anyhow!(
                            t!("error-invalid-argument", "argument" => &args[i])
                        ))
                    }
                };
            ...
            "--format" => {
                ...
                if i >= args.len() {
                    return Err(anyhow!(
                        t!("error-missing-argument", "option" => "--format")
                    ));
                }
            ...
            "--buffer-size" => {
                ...
                if i >= args.len() {
                    return Err(anyhow!(
                        t!("error-missing-argument", "option" => "--buffer-size")
                    ));
                }
            ...
            "--timeout" => {
                ...
                if i >= args.len() {
                    return Err(anyhow!(
                        t!("error-missing-argument", "option" => "--timeout")
                    ));
                }
...
fn process_stdin(options: &CatOptions) -> Result<()> {
    ...
    let stats = process_reader(Box::new(reader), &mut writer, options, "<stdin>")?;
    ...
    for (filename, result_rx) in file_results {
        match result_rx.recv()? {
            ...
            Err(e) => {
                eprintln!(
                    "{}",
                    t!("cat-error-file", "filename" => filename, "error" => e.to_string())
                );
            }
        }
    }
...
fn process_single_file(
    ...
    if _filename == "-" {
        ...
        return process_reader(Box::new(reader), &mut writer, options, "<stdin>");
    }
    ...
    let metadata = std::fs::metadata(path).context(t!("error-io-error"))?;
    ...
    let final_path = if options.follow_symlinks && metadata.file_type().is_symlink() {
        std::fs::canonicalize(path).context(t!("error-io-error"))?
    } else {
        path.to_path_buf()
    };
...
fn process_file_mmap<W: Write>(
    ...
    let file = File::open(path).context(t!("error-io-error"))?;
...
fn process_file_stream<W: Write + ?Sized>(
    ...
    let reader: Box<dyn BufRead> = match compression {
        ...
        None => Box::new(BufReader::with_capacity(options.buffer_size, file)),
    };

    let stats =
        process_reader_with_progress(reader, writer, options, filename, progress_bar.as_ref())?;
        ...
        match options.output_format {
            ...
            OutputFormat::Base64 => {
                write!(
                    writer,
                    "{}",
                    general_purpose::STANDARD.encode(line.as_bytes())
                )?;
            }
            OutputFormat::Json => {
...
fn process_url_to_writer(
    ...
    if scheme == "file" {
        if let Ok(path_buf) = url.to_file_path() {
            ...
            let compression = if options.decompress {
                detect_compression(&path_buf)?
            } else {
                None
            };
            return process_file_stream(
                &path_buf,
                writer,
                options,
        ...
        if let Some(comma_idx) = s.find(',') {
            ...
            let bytes = if is_base64 {
                general_purpose::STANDARD
                    ...
                    .map_err(|e| {
                        anyhow!(t!("cat-error-invalid-base64", "error" => e.to_string()))
                    })?
            } else {
            ...
            return process_reader_with_progress(Box::new(reader), writer, options, "data:", None);
        } else {
    ...
    if scheme == "ftp" {
        #[cfg(feature = "net-ftp")]
        {
            ...
            let host = url
                .host_str()
                .ok_or_else(|| anyhow!(t!("cat-error-invalid-ftp-host")))?;
            ...
            let mut ftp_stream = FtpStream::connect(format!("{host}:{port}")).with_context(
                || t!("cat-error-ftp-connect-failed", "host" => host, "port" => port.to_string()),
            )?;
    ...
    if scheme != "http" && scheme != "https" {
        return Err(anyhow!(
            t!("cat-error-unsupported-url-scheme", "scheme" => scheme)
        ));
    }
...
fn detect_compression(path: &Path) -> Result<Option<CompressionType>> {
    ...
    match extension.as_deref() {
        ...
        _ => {
            ...
            if bytes_read >= 4 && buffer[0..4] == [0x28, 0xb5, 0x2f, 0xfd] {
...
fn print_help() {
    ...
    println!(
        "  -A, --show-all           {}",
        t!("cat-help-option-show-all")
    );
    println!(
        "  -b, --number-nonblank    {}",
        t!("cat-help-option-number-nonblank")
    );
    println!(
        "  -e                       {}",
        t!("cat-help-option-e-short-desc")
    );
    println!(
        "  -E, --show-ends          {}",
        t!("cat-help-option-show-ends")
    );
    println!(
        "  -n, --number             {}",
        t!("cat-help-option-number")
    );
    println!(
        "  -s, --squeeze-blank      {}",
        t!("cat-help-option-squeeze-blank")
    );
    println!(
        "  -t                       {}",
        t!("cat-help-option-t-short-desc")
    );
    println!(
        "  -T, --show-tabs          {}",
        t!("cat-help-option-show-tabs")
    );
    println!(
        "  -u                       {}",
        t!("cat-help-option-u-ignored")
    );
    println!(
        "  -v, --show-nonprinting   {}",
        t!("cat-help-option-show-nonprinting")
    );
    println!();
...
mod tests {
    ...
    fn test_file_processing() -> Result<()> {
        ...
        let options = CatOptions::default();
        let stats = process_single_file(temp_file.path().to_str().unwrap(), &options, None)?;
    ...
    fn test_ftp_url_feature_disabled() -> Result<()> {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("FTP support is not enabled"));
        Ok(())
    }
