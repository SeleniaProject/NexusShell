...
pub fn while_cmd_cli(args: &[String]) -> Result<(), ShellError> {
    if args.is_empty() || args.contains(&"--help".to_string()) {
        print_help();
        return Ok(());
    }

    // Basic while loop implementation
    // This is a simplified version - full shell while would require complex parsing
    
    // Find the 'do' keyword
    let do_pos = args.iter().position(|arg| arg == "do")
        .ok_or_else(|| ShellError::new(ErrorKind::RuntimeError(RuntimeErrorKind::InvalidArgument), "while: missing 'do' keyword"))?;
    
    // Find the 'done' keyword
    let done_pos = args.iter().position(|arg| arg == "done")
        .ok_or_else(|| ShellError::new(ErrorKind::RuntimeError(RuntimeErrorKind::InvalidArgument), "while: missing 'done' keyword"))?;
    
    if do_pos >= done_pos {
        return Err(ShellError::new(ErrorKind::RuntimeError(RuntimeErrorKind::InvalidArgument), "while: invalid syntax - 'do' must come before 'done'"));
    }
    
    let condition_args = &args[..do_pos];
    let body_args = &args[do_pos + 1..done_pos];
    
    execute_while_loop(condition_args, body_args)
}

fn print_help() {
    println!("while - execute commands while condition is true
...
DESCRIPTION:
    Execute COMMANDS repeatedly as long as CONDITION evaluates to true.
...
fn execute_while_loop(condition_args: &[String], body_args: &[String]) -> Result<(), ShellError> {
    ...
    loop {
        ...
        if iteration_count > max_iterations {
            return Err(ShellError::new(ErrorKind::RuntimeError(RuntimeErrorKind::InvalidArgument), "while: maximum iteration limit reached (safety limit)"));
        }
