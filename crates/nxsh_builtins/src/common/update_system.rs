...
use nxsh_core::{nxsh_log_debug, nxsh_log_error, nxsh_log_info, nxsh_log_warn};
...
pub fn init_update_system(config: UpdateConfig) -> Result<()> {
    ...
    fs::create_dir_all(&config.cache_dir).context("Failed to create update cache directory")?;
    ...
    UPDATE_SYSTEM
        .set(system)
        .map_err(|_| anyhow!("Update system already initialized"))?;
...
pub fn force_bypass_cache(enable: bool) {
    BYPASS_CACHE.store(enable, Ordering::Relaxed);
}

/// Whether update system is initialized
pub fn is_initialized() -> bool {
    UPDATE_SYSTEM.get().is_some()
}
    ...
    match check_for_updates_internal(system).await {
        Ok(manifest) => {
            if let Some(manifest) = manifest {
                let should_download =
                    system.config.auto_download && !system.config.require_user_consent;
...
async fn check_for_updates_internal(system: &UpdateSystem) -> Result<Option<UpdateManifest>> {
    ...
    #[cfg(feature = "updates")]
    {
        retry_with_exponential_backoff(
            || async { check_for_updates_with_retry(system).await },
            system.config.max_retries,
            system.config.retry_delay_ms,
        )
        .await
    }
}
...
async fn check_for_updates_with_retry(system: &UpdateSystem) -> Result<Option<UpdateManifest>> {
    ...
    let mut manifest_url = format!(
        "{}/manifest/{}",
        system.config.update_server_url, channel_str
    );
    ...
    if let Err(e) = &response {
        return Err(anyhow!("Failed to fetch update manifest: {e}"));
    }
    ...
    let update_available = {
        let latest_version =
            Version::parse(&manifest.version).context("Failed to parse latest version")?;
        let current_version =
            Version::parse(&system.current_version).context("Failed to parse current version")?;
...
fn verify_manifest_signature(manifest: &UpdateManifest, public_key_str: &str) -> Result<()> {
    ...
    let public_key_vec = hex::decode(public_key_str).context("Failed to decode public key")?;
    if public_key_vec.len() != 32 {
        return Err(anyhow!(
            "Public key length invalid: expected 32 bytes got {}",
            public_key_vec.len()
        ));
    }
    ...
    // Parse signature
    let signature_bytes = hex::decode(&manifest.signature).context("Failed to decode signature")?;
    let signature =
        Signature::from_slice(&signature_bytes).map_err(|e| anyhow!("Invalid signature: {}", e))?;
...
async fn download_update(system: &UpdateSystem, manifest: &UpdateManifest) -> Result<PathBuf> {
    ...
    #[cfg(feature = "updates")]
    {
        ...
        let mut file =
            std::fs::File::create(&download_path).context("Failed to create download file")?;
...
#[cfg(feature = "updates")]
fn verify_file_checksum_with_retry(
    file_path: &Path,
        ...
        match verify_file_checksum_single_attempt(file_path, expected_checksum) {
            ...
            Err(e) => {
                ...
                if attempt < max_retries {
                    nxsh_log_warn!(
                        "Checksum verification failed on attempt {}, retrying...",
                        attempt + 1
                    );
...
fn verify_file_size(file_path: &Path, expected_size: Option<u64>) -> Result<()> {
    if let Some(expected) = expected_size {
        ...
        nxsh_log_info!(
            "File size verification passed for {:?}: {} bytes",
            file_path,
            actual_size
        );
    }
...
async fn perform_atomic_installation(target: &Path, source: &Path) -> Result<()> {
    ...
    // Copy new binary
    fs::copy(source, target).context("Failed to copy update during fallback installation")?;
    ...
    #[cfg(unix)]
    {
        ...
        let mut perms = fs::metadata(target)
            ...
            .permissions();
        perms.set_mode(0o755);
        fs::set_permissions(target, perms).context("Failed to set executable permissions")?;
    }
...
async fn perform_rollback(target: &Path, backup: &Path) -> Result<()> {
    ...
    fs::copy(backup, target).context("Failed to restore backup during fallback rollback")?;
...
fn check_cached_update_info(current_version: &str) -> Result<bool> {
    ...
    nxsh_log_debug!(
        "Checking version {} against cached manifest",
        current_version
    );
...
pub mod management {
    ...
    pub fn get_detailed_status() -> Result<UpdateSystemStatus> {
        ...
        system_metrics.insert(
            "config_channel".to_string(),
            format!("{:?}", system.config.channel),
        );
        system_metrics.insert(
            "auto_download".to_string(),
            system.config.auto_download.to_string(),
        );
        system_metrics.insert(
            "auto_install".to_string(),
            system.config.auto_install.to_string(),
        );
        system_metrics.insert(
            "backup_count".to_string(),
            system.config.backup_count.to_string(),
        );
        system_metrics.insert(
            "max_retries".to_string(),
            system.config.max_retries.to_string(),
        );
        system_metrics.insert(
            "timeout_seconds".to_string(),
            system.config.timeout_seconds.to_string(),
        );
...
mod tests {
    ...
    async fn test_atomic_installation_fallback() {
        ...
        // Create source file
        tokio::fs::write(&source, b"Updated binary content")
            .await
            .unwrap();

        // Create existing target for backup test
        tokio::fs::write(&target, b"Original binary content")
            .await
            .unwrap();
    ...
    async fn test_rollback_functionality() {
        ...
        // Create files
        tokio::fs::write(&target, b"Corrupted content")
            .await
            .unwrap();
        tokio::fs::write(&backup, b"Original content")
            .await
            .unwrap();
