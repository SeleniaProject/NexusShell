...
pub fn build_literals_huffman(data: &[u8]) -> Option<(HuffmanTable, Vec<u8>)> {
    ...
    for &b in data { freq[b as usize] = freq[b as usize].saturating_add(1); }
    ...
    if present_count <= 1 { return None; }
    // Direct-weights headerは最高リテラル値が127以下でないと不可
    if last_sym > 127 { return None; }
    ...
    #[derive(Clone, Copy)]
    struct Node { freq: u32, sym: i16, left: i16, right: i16 }
    let mut nodes: Vec<Node> = Vec::new();
    for i in 0..=last_sym { if freq[i] > 0 { nodes.push(Node{ freq: freq[i], sym: i as i16, left: -1, right: -1 }); } }
    if nodes.len() == 1 { return None; }
    // Min-heap of indices into nodes
    use std::cmp::Ordering;
    #[derive(Clone, Copy)]
    struct HeapItem { freq: u32, idx: usize }
    impl PartialEq for HeapItem { fn eq(&self, o: &Self) -> bool { self.freq == o.freq && self.idx == o.idx } }
    impl Eq for HeapItem {}
    impl PartialOrd for HeapItem { fn partial_cmp(&self, o: &Self) -> Option<Ordering> { Some(self.cmp(o)) } }
    impl Ord for HeapItem { fn cmp(&self, o: &Self) -> Ordering { // reverse for min-heap using BinaryHeap (max-heap)
    ...
    for (i, n) in nodes.iter().enumerate() { heap.push(HeapItem{ freq: n.freq, idx: i }); }
    // Build tree by adding internal nodes appended to nodes vector
    while heap.len() > 1 {
        let a = heap.pop().unwrap();
        let b = heap.pop().unwrap();
        let new_idx = nodes.len();
        let new_node = Node{ freq: a.freq.saturating_add(b.freq), sym: -1, left: a.idx as i16, right: b.idx as i16 };
        nodes.push(new_node);
        heap.push(HeapItem{ freq: nodes[new_idx].freq, idx: new_idx });
    }
    ...
    fn assign_depths(nodes: &Vec<Node>, idx: usize, depth: u8, lengths: &mut [u8;256]) {
        let n = nodes[idx];
        if n.sym >= 0 { lengths[n.sym as usize] = depth.max(1); return; }
        if n.left >= 0 { assign_depths(nodes, n.left as usize, depth+1, lengths); }
        if n.right >= 0 { assign_depths(nodes, n.right as usize, depth+1, lengths); }
    }
    ...
    if max_bits > 11 {
        ...
        let mut present: Vec<(usize, u32)> = (0..=last_sym).filter(|&i| freq[i] > 0).map(|i| (i, freq[i])).collect();
        present.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
        // Build unconstrained bl_count first
        let mut bl_count = [0i32; 64];
        for i in 0..=last_sym { let l = lengths[i]; if l > 0 { bl_count[l as usize] += 1; } }
        ...
        while cur_max > 11 {
            if bl_count[cur_max] > 0 {
                ...
                if bl_count[cur_max] < 0 { bl_count[cur_max] = 0; }
                continue;
            }
            cur_max -= 1;
        }
        // Ensure there are no counts below 1-bit negative
        for b in 1..=11 { if bl_count[b] < 0 { bl_count[b] = 0; } }
        ...
        for bits in 1..=11 {
            ...
            for _ in 0..cnt { if idx < present.len() { eff_lengths[present[idx].0] = bits as u8; idx += 1; } }
        }
        // If any leftover symbols, assign them to the deepest allowed
        while idx < present.len() { eff_lengths[present[idx].0] = 11; idx += 1; }
        lengths = eff_lengths;
        max_bits = 11;
    }
    ...
    for i in 0..=last_sym {
        ...
        weights[i] = if l == 0 { 0 } else { max_bits.saturating_add(1).saturating_sub(l) };
        // sanity: weights must be <= 15 (we're bounded by max_bits<=11)
        if weights[i] > 15 { return None; }
    }
    ...
    for i in (0..emit_weights).step_by(2) {
        ...
        let w1 = if i+1 < emit_weights { weights[i+1] & 0x0F } else { 0 };
        hdr.push((w0 << 4) | w1);
    }
    ...
    let mut sum: u32 = 0;
    for i in 0..last_sym { let w = weights[i] as u32; if w > 0 { sum = sum.saturating_add(1u32 << w); } }
    ...
    if leftover == 0 || (leftover & (leftover - 1)) != 0 {
        ...
        while try_bits < 11 {
            ...
            let mut sum2: u32 = 0;
            for i in 0..last_sym {
                let w = if weights[i] == 0 { 0 } else { (weights[i] as u32) + (try_bits as u32 - max_bits as u32) };
                if w > 0 { sum2 = sum2.saturating_add(1u32 << w); }
            }
        ...
        if !raised {
            ...
            if !found_fix { return None; }
        }
    }
    let w_last: u8 = leftover.trailing_zeros() as u8;
    if w_last > 15 { return None; }
    ...
    for i in 0..=last_sym {
        let w = eff_weights[i];
        eff_lengths[i] = if w == 0 { 0 } else { max_bits.saturating_add(1).saturating_sub(w) };
    }

    // Build canonical codes from eff_lengths
    let mut bl_count = [0u16; 32];
    let mut eff_max_bits = 0u8;
    for i in 0..=last_sym { let l = eff_lengths[i]; if l > 0 { bl_count[l as usize] += 1; if l > eff_max_bits { eff_max_bits = l; } } }
    ...
    Some((HuffmanTable { codes: codes_vec, last_symbol: last_sym, weights: eff_weights }, hdr))
}

pub fn build_fse_compressed_weights_header(_table: &HuffmanTable) -> Option<Vec<u8>> { None }
...
mod tests {
    ...
    assert!(maxb <= 127, "payload contains non-ASCII byte: {}", maxb);
        ...
        assert!(res.is_some(), "build_literals_huffman should succeed for ASCII payload <=127 symbols");
