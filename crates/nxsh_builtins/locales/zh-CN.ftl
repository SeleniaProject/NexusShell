...
# date 命令（元数据/相对时间/节假日）
date.error.invalid_timezone = 无效的时区：{$tz}
date.error.invalid_month = 无效的月份：{$month}

date.metadata.unix_timestamp = Unix 时间戳：{$value}
date.metadata.julian_day = 儒略日：{$value}
date.metadata.day_of_year = 年内第几天：{$value}
date.metadata.week_number = 周数：{$value}
date.metadata.weekday = 星期：{$value}
date.metadata.type.weekend = 类型：周末
date.metadata.type.business = 类型：工作日
date.metadata.astronomical = 天文：{$info}

date.relative.now = 现在
date.relative.minutes_ago = {$mins} 分钟前
date.relative.in_minutes = {$mins} 分钟后
date.relative.hours_ago = {$hours} 小时前
date.relative.in_hours = {$hours} 小时后
date.relative.days_ago = {$days} 天前
date.relative.in_days = {$days} 天后

date.holiday.none = 在年份 {$year}、地区 {$regions} 未找到节假日
date.holiday.header = 节假日 {$year} 年，地区：{$regions}
date.holiday.separator = =====================================
date.holiday.entry = {$date} - {$name}（{$region}, {$kind}）
date.holiday.total = 合计：{$count} 个节假日