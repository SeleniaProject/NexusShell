...
use nxsh_core::performance::{PerformanceOptimizer, PerformanceConfig};
...
impl BuiltinManager {
    fn new() -> Self { BuiltinManager }
    fn is_builtin(&self, cmd: &str) -> bool {
        matches!(cmd, "ls" | "cd" | "pwd" | "cat" | "grep" | "find" | "ps" | "top")
    }
}
...
mod tests {
    ...
    fn test_completion_performance() {
        ...
        assert!(!completions.is_empty());
        assert!(duration < Duration::from_millis(1), 
            "Completion took {:?}, should be <1ms", duration);
    ...
    fn test_builtin_manager_performance() {
        ...
        assert!(init_time < Duration::from_millis(5),
            "BuiltinManager initialization took {:?}, should be <5ms", init_time);
        ...
        println!("BuiltinManager init: {:?}, 1000 lookups: {:?}", init_time, lookup_time);
    }
    ...
    fn test_memory_efficiency() {
        ...
        assert!(memory_per_manager < 5 * 1024 * 1024, // 5MB per manager max
