...
fn test_error_handling_user_functions() {
    ...
    assert!(result
        .unwrap_err()
        .contains("negative numbers not supported"));

    // Test factorial with non-integer
    let result =
        executor.call_user_function_by_name("factorial", vec![MirValue::String("abc".to_string())]);
    assert!(result.is_err());
    ...
    let result =
        executor.call_user_function_by_name("map", vec![MirValue::String("double".to_string())]);
    assert!(result.is_err());
...
fn test_boundary_values() {
    ...
    // Test factorial(0) = 1
    let result = executor
        .call_user_function_by_name("factorial", vec![MirValue::Integer(0)])
        .unwrap();
    assert_eq!(result, MirValue::Integer(1));

    // Test fibonacci(0) = 0
    let result = executor
        .call_user_function_by_name("fibonacci", vec![MirValue::Integer(0)])
        .unwrap();
    assert_eq!(result, MirValue::Integer(0));

    // Test fibonacci(1) = 1
    let result = executor
        .call_user_function_by_name("fibonacci", vec![MirValue::Integer(1)])
        .unwrap();
    assert_eq!(result, MirValue::Integer(1));
