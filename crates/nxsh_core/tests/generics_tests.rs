...
fn generic_function_multiple_specializations() {
    ...
    assert_eq!(
        ctx.ensure_monomorphized("wrap", &["str"]).as_deref(),
        Some("wrap__gen_str")
    );
    assert_eq!(
        ctx.ensure_monomorphized("wrap", &["num"]).as_deref(),
        Some("wrap__gen_num")
    );
    let parser = Parser::new();
    let ast = parser
        .parse("wrap__gen_str hello; wrap__gen_num 7")
        .expect("parse failed");
    let res = ex.execute(&ast, &mut ctx).expect("execute failed");
    assert_eq!(res.exit_code, 0);
}
