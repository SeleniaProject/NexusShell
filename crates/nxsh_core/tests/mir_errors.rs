...
use nxsh_parser::ast::{AssignmentOperator, AstNode, NumberType};
...
fn error_div_by_zero() {
    ...
    let closure = AstNode::Closure {
        params: vec![],
        body: Box::new(AstNode::Return(Some(Box::new(expr)))),
        captures: vec![],
        is_async: false,
    };
    ...
    assert!(
        matches!(result, Err(nxsh_core::mir::MirError::DivByZero)),
        "expected DivByZero got {result:?}"
    );
}
...
fn error_type_mismatch_arith() {
    ...
    // "a" + 1 -> type mismatch
    let expr = AstNode::BinaryExpression {
        left: Box::new(AstNode::StringLiteral {
            value: "a",
            quote_type: nxsh_parser::ast::QuoteType::Double,
        }),
        operator: BO::Add,
        right: Box::new(AstNode::NumberLiteral {
            value: "1",
    ...
    let result = run(vec![call]);
    assert!(
        matches!(result, Err(nxsh_core::mir::MirError::TypeMismatch(_))),
...
fn error_regex_compile() {
    ...
    assert!(
        matches!(result, Err(nxsh_core::mir::MirError::RegexCompile(_, _))),
        "expected RegexCompile error got {result:?}"
    );
}
...
fn error_regex_type_mismatch() {
    ...
    assert!(
        matches!(result, Err(nxsh_core::mir::MirError::TypeMismatch(_))),
        "expected regex TypeMismatch got {result:?}"
    );
}
