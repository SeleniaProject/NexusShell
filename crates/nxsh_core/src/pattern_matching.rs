...
use nxsh_parser::ast::{AstNode, BinaryOperator, MatchArm, Pattern, UnaryOperator};
...
impl PatternMatchingEngine {
    ...
    fn compile_pattern_recursive(
        ...
        match pattern {
            ...
            Pattern::Guard { pattern, condition } => {
                self.compile_pattern_recursive(pattern, bytecode, bindings, is_refutable)?;
                bytecode.push(PatternInstruction::EvaluateGuard(unsafe {
                    std::mem::transmute::<AstNode<'_>, AstNode<'_>>(condition.as_ref().clone())
                }));
                *is_refutable = true;
            }
    ...
    fn execute_pattern(
        ...
        debug!(
            bytecode_len = pattern.bytecode.len(),
            "Executing pattern bytecode"
        );
        ...
        while pc < pattern.bytecode.len() {
            match &pattern.bytecode[pc] {
                ...
                _ => {
                    ...
                    warn!(instruction = ?pattern.bytecode[pc], "Unhandled pattern instruction");
                    // Fixed ? syntax
                }
            }
    ...
    fn evaluate_guard(
        &self,
        guard: &AstNode,
        bindings: &HashMap<String, PatternValue>,
    ) -> ShellResult<bool> {
        debug!(
            ?guard,
            bindings_count = bindings.len(),
            "Evaluating guard clause"
        );
    ...
    fn check_exhaustiveness(&self, arms: &[MatchArm]) -> ShellResult<bool> {
        ...
        let has_wildcard = arms
            .iter()
            .any(|arm| matches!(&arm.pattern, Pattern::Wildcard | Pattern::Placeholder));
...
impl PatternMatchingEngine {
                ...
                match name.as_ref() {
                    AstNode::Variable("len") => {
                        if args.len() == 1 {
                            ...
                            Ok(PatternValue::Integer(
                                self.get_pattern_value_length(&arg_val) as i64,
                            ))
                        } else {
    ...
    fn apply_binary_operation(
        ...
        match operator {
            ...
            LogicalAnd => Ok(Value::Boolean(
                self.is_truthy(left) && self.is_truthy(right),
            )),
            LogicalOr => Ok(Value::Boolean(
                self.is_truthy(left) || self.is_truthy(right),
            )),
    ...
    fn guard_value_to_string(value: &Value) -> String {
        match value {
            ...
            Value::Array(arr) => format!(
                "[{}]",
                arr.iter()
                    .map(Self::guard_value_to_string)
                    .collect::<Vec<_>>()
                    .join(", ")
            ),
    ...
    fn apply_binary_operation_pattern(
        &self,
        left: &PatternValue,
        operator: &BinaryOperator,
        right: &PatternValue,
        ...
        match operator {
            ...
            LogicalAnd => Ok(PatternValue::Boolean(
                self.is_pattern_truthy(left) && self.is_pattern_truthy(right),
            )),
            LogicalOr => Ok(PatternValue::Boolean(
                self.is_pattern_truthy(left) || self.is_pattern_truthy(right),
            )),
            _ => Err(crate::error::ShellError::new(
                crate::error::ErrorKind::RuntimeError(
    ...
    fn apply_unary_operation_pattern(
        &self,
        operator: &UnaryOperator,
    ...
    fn add_pattern_values(
        ...
        match (left, right) {
            ...
            (PatternValue::String(a), PatternValue::String(b)) => {
                Ok(PatternValue::String(format!("{a}{b}")))
            }
            _ => Err(crate::error::ShellError::new(
                crate::error::ErrorKind::RuntimeError(
                    crate::error::RuntimeErrorKind::InvalidArgument,
        ...
        match (left, right) {
            (PatternValue::Integer(a), PatternValue::Integer(b)) => {
                if *b == 0 {
                    Err(crate::error::ShellError::new(
                        crate::error::ErrorKind::RuntimeError(
                            crate::error::RuntimeErrorKind::DivisionByZero,
                        ),
                        "Division by zero",
                    ))
                } else {
    ...
    fn compare_pattern_values(
        ...
        match (left, right) {
            ...
            (PatternValue::Integer(a), PatternValue::Number(b)) => Ok((*a as f64)
                .partial_cmp(b)
                .unwrap_or(std::cmp::Ordering::Equal)
                as i32),
            (PatternValue::Number(a), PatternValue::Integer(b)) => {
