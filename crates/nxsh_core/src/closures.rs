...
impl ClosureSystem {
    ...
    pub fn call(&mut self, name: &str, args: Vec<Value>) -> Result<Value> {
        ...
        Err(crate::compat::anyhow(format!(
            "Function or closure '{name}' not found"
        )))
    }
    ...
    fn call_function(&mut self, function: &Function, args: Vec<Value>) -> Result<Value> {
        match function {
            Function::Builtin(builtin_fn) => builtin_fn.call(args),
    ...
    pub fn evaluate_expression(&mut self, expr: &Expression) -> Result<Value> {
        match expr {
            ...
            Expression::Lambda { params, body } => {
                ...
                let closure_id =
                    self.create_closure(params.clone(), (**body).clone(), captured_vars)?;
            ...
            Expression::Assignment { var_name, value } => {
                ...
                self.execution_context
                    .set_variable(var_name.clone(), evaluated_value.clone());
                Ok(evaluated_value)
            }
        }
    }
    ...
    fn register_builtin_functions(&mut self) {
        ...
        self.global_functions.insert(
            "add".to_string(),
            Function::Builtin(BuiltinFunction::new("add", |args| {
                if args.len() == 2 {
                    match (&args[0], &args[1]) {
        ...
        self.global_functions.insert(
            "multiply".to_string(),
            Function::Builtin(BuiltinFunction::new("multiply", |args| {
                if args.len() == 2 {
                    match (&args[0], &args[1]) {
                        (Value::Integer(a), Value::Integer(b)) => Ok(Value::Integer(a * b)),
                        (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a * b)),
        ...
        self.global_functions.insert(
            "length".to_string(),
            Function::Builtin(BuiltinFunction::new("length", |args| {
                if args.len() == 1 {
                    match &args[0] {
        ...
        self.global_functions.insert(
            "uppercase".to_string(),
            Function::Builtin(BuiltinFunction::new("uppercase", |args| {
                if args.len() == 1 {
                    match &args[0] {
        ...
        match op {
            ...
            BinaryOperator::Divide => match (left, right) {
                ...
                (Value::Float(_), Value::Float(b)) if *b == 0.0 => {
                    Err(crate::anyhow!("Division by zero"))
                }
                (Value::Float(a), Value::Float(b)) => Ok(Value::Float(a / b)),
            ...
            BinaryOperator::And => Ok(Value::Boolean(
                self.value_to_bool(left) && self.value_to_bool(right),
            )),
...
impl ExecutionContext {
    ...
    pub fn push_scope(&mut self, scope: ExecutionScope) {
        self.scope_stack
            .push(std::mem::replace(&mut self.current_scope, scope));
    }
...
mod tests {
    ...
    fn test_basic_function_call() {
        ...
        let result = system
            .call("add", vec![Value::Integer(2), Value::Integer(3)])
            .unwrap();
