...
pub enum MirInstruction {
    ...
    ClosureCreate {
        ...
        capture_regs: Vec<MirRegister>,
        param_regs: Vec<MirRegister>,
...
impl fmt::Display for MirInstruction {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MirInstruction::LoadImmediate { dest, value } => write!(f, "{dest} = load {value}"),
            MirInstruction::Move { dest, src } => write!(f, "{dest} = move {src}"),
            MirInstruction::Load { dest, source } => write!(f, "{dest} = load ${source}"),
            MirInstruction::Store { dest, value } => write!(f, "store ${dest}, {value}"),
            MirInstruction::Add { dest, left, right } => write!(f, "{dest} = add {left}, {right}"),
            MirInstruction::Call {
                dest,
                function,
                args,
...
impl MirFunction {
    ...
    pub fn get_block_mut(&mut self, id: u32) -> Option<&mut MirBasicBlock> {
        self.blocks
            .entry(id)
            .or_insert_with(|| MirBasicBlock::new(id));
...
impl fmt::Display for MirProgram {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            f,
            "MIR Program (optimization level: {})",
...
impl std::fmt::Display for MirError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MirError::RegexCompile(p, e) => {
                write!(f, "regex compile failed for pattern '{p}': {e}")
            }
...
impl MirError {
    ...
    pub fn contains(&self, needle: &str) -> bool {
        match self {
            ...
            MirError::DivByZero => {
                needle.eq_ignore_ascii_case("division by zero")
                    || needle.eq_ignore_ascii_case("div by zero")
            }
        }
    }
}
...
impl MirExecutor {
    ...
    pub fn execute(&mut self, program: &MirProgram) -> Result<MirValue, MirError> {
        ...
        let main_function = match &program.main_function {
            Some(name) => program
                .get_function(name)
                .ok_or_else(|| MirError::Runtime("Main function not found".into()))?,
                    ...
                    if let Some(frame) = self.call_stack.pop() {
                        if frame.is_closure {
                            if let Some(ret_reg) = frame.return_register {
                                ...
                                if let MirValue::Register(r) = MirValue::Register(ret_reg.clone()) {
                                    let _ = self.set_register(&r, value.clone());
                                }
                            }
                            return Ok(value);
                        } else {
    ...
    fn execute_instruction(
        ...
        match instruction {
            ...
            MirInstruction::GreaterThan { dest, left, right } => {
                let lv = self.get_value(left)?;
                let rv = self.get_value(right)?;
            ...
            MirInstruction::Return { value } => {
                ...
                if let Some(frame) = self.call_stack.last() {
                    if frame.is_closure {
                        return Ok(InstructionResult::Return(return_value));
                    }
                }
            ...
            MirInstruction::ClosureCall {
                ...
                if let MirValue::Object(map) = clo {
                    if let Some(MirValue::Integer(block_id)) = map.get("__closure_block") {
                        ...
                        if let (
                            Some(MirValue::Array(cap_arr)),
                            Some(MirValue::Array(cap_regs_arr)),
                        ) = (map.get("captures"), map.get("capture_regs"))
                        {
                ...
                match (array_val, index_val) {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "Array access requires array and integer".into(),
                        ))
                    }
                }
                Ok(InstructionResult::Continue)
            ...
            MirInstruction::ArraySet {
                ...
                match (array_val, index_val) {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "Array assignment requires array and integer".into(),
                        ))
                    }
                }
                Ok(InstructionResult::Continue)
            ...
            MirInstruction::ObjectGet {
                ...
                match object_val {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "Object access requires object".into(),
                        ))
                    }
                }
                Ok(InstructionResult::Continue)
            }
            MirInstruction::ObjectSet {
                object,
                field,
                value,
            } => {
                ...
                match object_val {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "Object assignment requires object".into(),
                        ))
                    }
                }
                Ok(InstructionResult::Continue)
            }
            MirInstruction::DefineFunction {
                name: _,
                function: _,
            } => {
            ...
            MirInstruction::GetIterator { dest, iterable } => {
                ...
                match iterable_val {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "Cannot iterate over non-iterable value".into(),
                        ))
                    }
                }
                Ok(InstructionResult::Continue)
            }
            MirInstruction::IteratorNext {
                iterator,
                element,
                has_next,
            } => {
                ...
                match iter_val {
                    MirValue::Integer(index) => {
                        ...
                        self.set_register(has_next, MirValue::Boolean(index < 10))?;
                        // Mock limit
                    }
                    _ => {
            ...
            MirInstruction::Unreachable => {
                Err(MirError::Runtime("Reached unreachable code".into()))
            }
    ...
    fn perform_arithmetic(
        ...
        match (left, right) {
            (MirValue::Integer(a), MirValue::Integer(b)) => match op {
            ...
            _ => Err(MirError::TypeMismatch(format!(
                "Invalid operands for arithmetic operation: {op} {left:?} {right:?}"
            ))),
        }
    }
    ...
    fn perform_comparison(
        ...
        match (left, right) {
            (MirValue::Integer(a), MirValue::Integer(b)) => match op {
                ...
                "ge" => Ok(a >= b),
                _ => Err(MirError::Runtime(format!(
                    "Unknown comparison operation: {op}"
                ))),
            },
            (MirValue::Float(a), MirValue::Float(b)) => match op {
            ...
            (MirValue::String(a), MirValue::String(b)) => match op {
                "eq" => Ok(a == b),
                "ne" => Ok(a != b),
                "lt" => Ok(a < b),
                "le" => Ok(a <= b),
            ...
            (MirValue::Boolean(a), MirValue::Boolean(b)) => match op {
                "eq" => Ok(a == b),
                "ne" => Ok(a != b),
                _ => Err(MirError::TypeMismatch(format!(
                    "Invalid comparison operation for booleans: {op}"
                ))),
            },
            _ => Err(MirError::TypeMismatch(format!(
                "Invalid operands for comparison: {op} {left:?} {right:?}"
            ))),
        }
    }
    ...
    fn builtin_head(&self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        let result: Vec<MirValue> = content
            .lines()
            .take(lines_count)
    ...
    fn builtin_awk(&self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        if program.contains("print $") {
            let field_num = if program.contains("$1") {
                1
            } else if program.contains("$2") {
                2
            } else if program.contains("$3") {
                3
            } else {
                1
            };
    ...
    fn builtin_xargs(&self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        for line in input.lines() {
            ...
            match std::process::Command::new(&command).arg(line).output() {
                Ok(output) => {
    ...
    fn execute_user_function_blocks(
        &mut self,
        function: &MirFunction,
    ) -> Result<MirValue, MirError> {
        let mut current_block_id = function.entry_block;
        ...
        loop {
            ...
            let block = function.get_block(current_block_id).ok_or(format!(
                "Block {} not found in function '{}'",
                current_block_id, function.name
            ))?;
    ...
    fn execute_user_function_instruction(
        &mut self,
        instruction: &MirInstruction,
    ) -> Result<InstructionResult, MirError> {
        self.stats.instructions_executed += 1;
        ...
        match instruction {
            ...
            MirInstruction::Store { dest, value } => {
                ...
                if let Some(call_frame) = self.call_stack.last_mut() {
                    call_frame
                        .local_variables
                        .insert(dest.clone(), resolved_value);
                } else {
    ...
    pub fn call_user_function_by_name(
        &mut self,
        function_name: &str,
        args: Vec<MirValue>,
    ) -> Result<MirValue, MirError> {
    ...
    pub fn execute_main(&mut self, program: &MirProgram) -> Result<MirValue, MirError> {
        ...
        while !visited.contains(&current_block_id) {
            ...
            let block = func
                ...
                .ok_or_else(|| MirError::Runtime(format!("block {current_block_id} missing")))?;
            let mut ip: usize = 0;
            while ip < block.instructions.len() {
                ...
                match inst {
                    ...
                    | LessThan { dest, left, right }
                    | LessEqual { dest, left, right }
                    | GreaterThan { dest, left, right }
                    | GreaterEqual { dest, left, right }
                    | And { dest, left, right }
                    | Or { dest, left, right } => {
                    ...
                    | OrSC {
                        ...
                        if !need_right {
                            ...
                            let idx = dest.id() as usize;
                            self.ensure_register_capacity(idx + 1);
                            self.registers[idx] = MirValue::Boolean(l_bool);
                            ip += *skip as usize + 1;
                            continue;
                        } else {
                    ...
                    RegexMatch {
                        ...
                        let v_val = resolve(value, &self.registers);
                        let p_val = resolve(pattern, &self.registers);
                        let result = match (v_val.clone(), p_val.clone()) {
                            (MirValue::String(s), MirValue::String(pat)) => {
                                match regex::Regex::new(&pat) {
                                    Ok(re) => MirValue::Boolean(re.is_match(&s) ^ *not),
                                    Err(e) => {
                                        return Err(MirError::RegexCompile(pat, e.to_string()))
                                    }
                                }
                            }
                            _ => {
                                return Err(MirError::TypeMismatch(
                                    "regex operands must be strings".into(),
                                ))
                            }
                        };
                        let idx = dest.id() as usize;
                    ...
                    ClosureCall {
                        ...
                        let closure_val = match closure {
                            MirValue::Register(r) => self
                                .registers
                                .get(r.id() as usize)
                                .cloned()
                                .unwrap_or(MirValue::Null),
                            other => other.clone(),
                        };
                        if let MirValue::Object(obj) = closure_val {
                            if let (
                                Some(MirValue::Integer(block_id_i)),
                                Some(MirValue::Array(cap_vals)),
                                Some(MirValue::Array(capture_regs_vals)),
                                Some(MirValue::Array(param_regs_vals)),
                            ) = (
                                obj.get("block"),
                                obj.get("captures"),
                                obj.get("capture_regs"),
                                obj.get("param_regs"),
                            ) {
                                // Map captures
                                for (cv, target_rv) in cap_vals.iter().zip(capture_regs_vals.iter())
                                {
                                    if let MirValue::Register(target_r) = target_rv {
                                        let resolved = match cv {
                                            MirValue::Register(src_r) => self
                                                .registers
                                                .get(src_r.id() as usize)
                                                .cloned()
                                                .unwrap_or(MirValue::Null),
                                            v => v.clone(),
                                        };
                                        let t_idx = target_r.id() as usize;
                                        self.ensure_register_capacity(t_idx + 1);
                                        self.registers[t_idx] = resolved;
                                    }
                                }
                                // Map args
                                for (arg, target_rv) in args.iter().zip(param_regs_vals.iter()) {
                                    if let MirValue::Register(target_r) = target_rv {
                                        ...
                                        let t_idx = target_r.id() as usize;
                                        self.ensure_register_capacity(t_idx + 1);
                                        self.registers[t_idx] = resolved_arg;
                                    }
                                }
                                    ...
                                    while ip2 < inner_block.instructions.len() {
                                        ...
                                        match inner_inst {
                                            ...
                                            | Or { dest, left, right } => {
                                                ...
                                                let r_val = resolve(right, &self.registers);
                                                let result =
                                                    self.eval_binary(inner_inst, &l_val, &r_val)?;
                                                let idx = dest.id() as usize;
                                                self.ensure_register_capacity(idx + 1);
                                                self.registers[idx] = result;
                                            }
                                            ...
                                            RegexMatch {
                                                ...
                                                let result = match (v_val.clone(), p_val.clone()) {
                                                    (
                                                        ...
                                                        Ok(re) => MirValue::Boolean(
                                                            re.is_match(&s) ^ *not,
                                                        ),
                                                        Err(e) => {
                                                            return Err(MirError::RegexCompile(
                                                                pat,
                                                                e.to_string(),
                                                            ))
                                                        }
                                ...
                                let d_idx = dest.id() as usize;
                                self.ensure_register_capacity(d_idx + 1);
                                self.registers[d_idx] = last_value.clone();
                            }
                        }
                    }
                    Return { value } => {
    ...
    fn eval_binary(
        ...
        use MirInstruction::*;
        let int = |mv: &MirValue| -> Option<i64> {
            if let MirValue::Integer(i) = mv {
                Some(*i)
            } else {
                None
            }
        };
        let boolv = |mv: &MirValue| -> Option<bool> {
            if let MirValue::Boolean(b) = mv {
                Some(*b)
            } else {
                None
            }
        ...
        match inst {
            ...
            | Shr { .. } => {
                ...
                if let (Some(a), Some(b)) = (a, b) {
                    ...
                    Err(MirError::TypeMismatch(
                        "binary arithmetic expects integers".into(),
                    ))
                }
            }
            ...
            LessThan { .. } | LessEqual { .. } | GreaterThan { .. } | GreaterEqual { .. } => {
                if let (Some(a), Some(b)) = (int(l_val), int(r_val)) {
                    Ok(MirValue::Boolean(match inst {
                        LessThan { .. } => a < b,
                        LessEqual { .. } => a <= b,
                        GreaterThan { .. } => a > b,
                        GreaterEqual { .. } => a >= b,
                    ...
                    Err(MirError::TypeMismatch("comparison expects integers".into()))
                }
            }
            And { .. } | Or { .. } | AndSC { .. } | OrSC { .. } => {
                if let (Some(a), Some(b)) = (boolv(l_val), boolv(r_val)) {
                    ...
                    Err(MirError::TypeMismatch("logical expects booleans".into()))
                }
            }
            _ => Ok(MirValue::Null),
        }
    ...
    fn execute_closure_object(
        &mut self,
        func: &MirFunction,
        obj: &std::collections::HashMap<String, MirValue>,
    ) -> Result<MirValue, MirError> {
        if let (
            Some(MirValue::Integer(block_id_i)),
            Some(MirValue::Array(cap_vals)),
            Some(MirValue::Array(capture_regs_vals)),
            Some(MirValue::Array(_param_regs_vals)),
        ) = (
            obj.get("block"),
            obj.get("captures"),
            obj.get("capture_regs"),
                ...
                while ip < inner_block.instructions.len() {
                    ...
                    match inner_inst {
                        ...
                        | MirInstruction::OrSC {
                            ...
                            let need_right = matches!(inner_inst, MirInstruction::AndSC { .. })
                                && l_bool
                        ...
                        MirInstruction::RegexMatch {
                            ...
                            let matched = match (v, p) {
                                (MirValue::String(s), MirValue::String(pat)) => {
                                    let re = regex::Regex::new(&pat)
                                        .map_err(|_| MirError::Runtime("invalid regex".into()))?;
                                    re.is_match(&s)
                                }
                                _ => {
                                    return Err(MirError::TypeMismatch(
                                        "regex expects strings".into(),
                                    ))
                                }
                            };
                            ...
                            let idx = dest.id() as usize;
                            self.ensure_register_capacity(idx + 1);
                            self.registers[idx] = MirValue::Boolean(res);
                        }
    ...
    fn builtin_factorial(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        if args.len() != 1 {
            return Err(MirError::Runtime(
                "factorial: requires exactly 1 argument".into(),
            ));
        }
        ...
        match &args[0] {
            MirValue::Integer(n) => {
                if *n < 0 {
                    return Err(MirError::TypeMismatch(
                        "factorial: negative numbers not supported".into(),
                    ));
                }
    ...
    fn builtin_fibonacci(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        if args.len() != 1 {
            return Err(MirError::Runtime(
                "fibonacci: requires exactly 1 argument".into(),
            ));
        }
        ...
        match &args[0] {
            MirValue::Integer(n) => {
                if *n < 0 {
                    return Err(MirError::TypeMismatch(
                        "fibonacci: negative numbers not supported".into(),
                    ));
                }
            ...
            _ => Err(MirError::TypeMismatch(
                "fibonacci: argument must be an integer".into(),
            )),
        }
    }
    ...
    fn builtin_max(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        for arg in &args[1..] {
            match (max_val, arg) {
                ...
                _ => {
                    return Err(MirError::TypeMismatch(
                        "max: all arguments must be the same numeric type".into(),
                    ))
                }
            }
        }
    ...
    fn builtin_min(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        for arg in &args[1..] {
            match (min_val, arg) {
                ...
                _ => {
                    return Err(MirError::TypeMismatch(
                        "min: all arguments must be the same numeric type".into(),
                    ))
                }
            }
        }
    ...
    fn builtin_sum(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        ...
        for arg in &args {
            match arg {
                ...
                _ => {
                    return Err(MirError::TypeMismatch(
                        "sum: arguments must be numeric or arrays".into(),
                    ))
                }
            }
        }
    ...
    fn builtin_map(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        if args.len() != 2 {
            return Err(MirError::Runtime(
                "map: requires exactly 2 arguments (function_name, array)".into(),
            ));
        }
        ...
        let array = match &args[1] {
            ...
            _ => {
                return Err(MirError::TypeMismatch(
                    "map: second argument must be an array".into(),
                ))
            }
        };
    ...
    fn builtin_filter(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        if args.len() != 2 {
            return Err(MirError::Runtime(
                "filter: requires exactly 2 arguments (predicate_name, array)".into(),
            ));
        }
        ...
        let array = match &args[1] {
            ...
            _ => {
                return Err(MirError::TypeMismatch(
                    "filter: second argument must be an array".into(),
                ))
            }
        };
        ...
        for element in array {
            ...
            let should_include = match predicate_name.as_str() {
                "isPositive" => match element {
                    MirValue::Integer(i) => *i > 0,
                    MirValue::Float(f) => *f > 0.0,
                    _ => false,
                },
                "isEven" => match element {
    ...
    fn builtin_reduce(&mut self, args: Vec<MirValue>) -> Result<MirValue, MirError> {
        if args.len() < 2 || args.len() > 3 {
            return Err(MirError::Runtime(
                "reduce: requires 2-3 arguments (function_name, array, [initial_value])".into(),
            ));
        }
        ...
        let array = match &args[1] {
            ...
            _ => {
                return Err(MirError::TypeMismatch(
                    "reduce: second argument must be an array".into(),
                ))
            }
        };
        ...
        if array.is_empty() {
            return if args.len() == 3 {
                ...
                Err(MirError::Runtime(
                    "reduce: empty array requires initial value".into(),
                ))
            };
        }
            ...
            accumulator = match function_name.as_str() {
                "add" => match (&accumulator, element) {
                    ...
                    (MirValue::String(a), MirValue::String(b)) => {
                        MirValue::String(format!("{a}{b}"))
                    }
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "add reduction requires compatible types".into(),
                        ))
                    }
                },
                "multiply" => match (&accumulator, element) {
                    (MirValue::Integer(a), MirValue::Integer(b)) => MirValue::Integer(a * b),
                    (MirValue::Float(a), MirValue::Float(b)) => MirValue::Float(a * b),
                    (MirValue::Integer(a), MirValue::Float(b)) => MirValue::Float(*a as f64 * b),
                    (MirValue::Float(a), MirValue::Integer(b)) => MirValue::Float(a * *b as f64),
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "multiply reduction requires numeric types".into(),
                        ))
                    }
                },
                "max" => match (&accumulator, element) {
                    (MirValue::Integer(a), MirValue::Integer(b)) => {
                        if b > a {
                            element.clone()
                        } else {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "max reduction requires same numeric types".into(),
                        ))
                    }
                },
                "min" => match (&accumulator, element) {
                    (MirValue::Integer(a), MirValue::Integer(b)) => {
                        if b < a {
                            element.clone()
                        } else {
                    ...
                    _ => {
                        return Err(MirError::TypeMismatch(
                            "min reduction requires same numeric types".into(),
                        ))
                    }
                },
                _ => {
                    // Try to call user-defined reduction function
                    self.call_user_function_by_name(
                        &function_name,
    ...
    fn execute_external_command(
        ...
        let string_args: Vec<String> = args.iter().map(|arg| self.value_to_string(arg)).collect();
            ...
            .output()
        {
            ...
            Err(e) => Err(MirError::Runtime(format!(
                "{command}: command not found: {e}"
            ))),
        }
    }
...
mod tests {
    ...
    fn test_mir_value_display() {
        ...
        assert_eq!(
            format!("{}", MirValue::String("hello".to_string())),
            "\"hello\""
        );
    ...
    fn test_comprehensive_instruction_set() {
        ...
        let instructions = vec![
            MirInstruction::LoadImmediate {
                dest: reg0.clone(),
                value: MirValue::Integer(42),
            },
            MirInstruction::Add {
                dest: reg1.clone(),
                left: MirValue::Register(reg0.clone()),
                right: MirValue::Integer(8),
            },
            MirInstruction::Call {
                dest: reg2.clone(),
                function: "echo".to_string(),
                args: vec![MirValue::Register(reg1)],
            },
    ...
    fn test_mir_executor_arithmetic() {
        ...
        // Test addition
        let result = executor
            .perform_arithmetic(&MirValue::Integer(5), &MirValue::Integer(3), "add")
            .expect("arithmetic operation should succeed");
        assert_eq!(result, MirValue::Integer(8));

        // Test multiplication
        let result = executor
            .perform_arithmetic(&MirValue::Integer(4), &MirValue::Integer(7), "mul")
            .expect("arithmetic operation should succeed");
    ...
    fn test_mir_executor_comparison() {
        ...
        assert!(executor
            .perform_comparison(&MirValue::Integer(5), &MirValue::Integer(3), "gt")
            .expect("comparison should succeed"));

        assert!(executor
            .perform_comparison(&MirValue::Integer(3), &MirValue::Integer(3), "eq")
            .expect("comparison should succeed"));
    ...
    fn test_mir_execution_simple_program() {
        ...
        let result = executor
            .execute(&program)
            .expect("program execution should succeed");
    ...
    fn test_mir_execution_conditional_program() {
        ...
        entry_block.add_instruction(MirInstruction::Jump { target: 1 });
    ...
    fn test_closure_create_and_call_basic() {
        ...
        closure_block.add_instruction(MirInstruction::Add {
            dest: reg_ret.clone(),
            left: MirValue::Register(reg_cap_target.clone()),
            right: MirValue::Register(reg_param.clone()),
        });
        closure_block.add_instruction(MirInstruction::Return {
            value: Some(MirValue::Register(reg_ret.clone())),
        });
        ...
        entry.add_instruction(MirInstruction::LoadImmediate {
            dest: reg_cap_source.clone(),
            value: MirValue::Integer(100),
        });
        // クロージャ生成: capture に reg_cap_source、capture_regs はクロージャ内で使われる reg_cap_target
        entry.add_instruction(MirInstruction::ClosureCreate {
            dest: reg_closure.clone(),
            func_block: 1,
            captures: vec![MirValue::Register(reg_cap_source.clone())],
            capture_regs: vec![reg_cap_target.clone()],
            param_regs: vec![reg_param.clone()],
            param_names: vec!["x".to_string()],
        });
        // 呼出し引数 23 をロードして closure call
        entry.add_instruction(MirInstruction::LoadImmediate {
            dest: reg_param.clone(),
            value: MirValue::Integer(23),
        }); // この値は call 時に上書きされるがテスト用に前値設定
        entry.add_instruction(MirInstruction::ClosureCall {
            dest: reg_result.clone(),
            closure: MirValue::Register(reg_closure.clone()),
            args: vec![MirValue::Integer(23)],
        });
        entry.add_instruction(MirInstruction::Return {
            value: Some(MirValue::Register(reg_result.clone())),
        });
    ...
    fn test_closure_argument_mismatch() {
        ...
        closure_block.add_instruction(MirInstruction::Add {
            dest: reg_ret.clone(),
            left: MirValue::Register(reg_cap_tgt.clone()),
            right: MirValue::Register(reg_param.clone()),
        });
        closure_block.add_instruction(MirInstruction::Return {
            value: Some(MirValue::Register(reg_ret.clone())),
        ...
        let mut entry = MirBasicBlock::new(0);
        entry.add_instruction(MirInstruction::LoadImmediate {
            dest: reg_cap_src.clone(),
            value: MirValue::Integer(5),
        });
        entry.add_instruction(MirInstruction::ClosureCreate {
            dest: reg_closure.clone(),
            func_block: 1,
            captures: vec![MirValue::Register(reg_cap_src.clone())],
            capture_regs: vec![reg_cap_tgt.clone()],
            param_regs: vec![reg_param.clone()],
    ...
    fn test_closure_return_instruction() {
        ...
        body.add_instruction(MirInstruction::Add {
            dest: reg_tmp.clone(),
            left: MirValue::Register(reg_cap_tgt.clone()),
            right: MirValue::Register(reg_param.clone()),
        });
        body.add_instruction(MirInstruction::ClosureReturn {
            value: Some(MirValue::Register(reg_tmp.clone())),
        });
        ...
        entry.add_instruction(MirInstruction::ClosureCall {
            dest: reg_res.clone(),
            closure: MirValue::Register(reg_closure.clone()),
            args: vec![MirValue::Integer(5)],
        });
        entry.add_instruction(MirInstruction::Return {
            value: Some(MirValue::Register(reg_res.clone())),
        });
        main_func.add_basic_block(entry);
        program.add_function(main_func);
        ...
        let result = executor
            .execute(&program)
            .expect("closure return should work");
