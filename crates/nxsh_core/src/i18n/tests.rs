...
"#;
    std::fs::write(locale_dir.join("en-US.ftl"), en_content)
...
"#;
    std::fs::write(locale_dir.join("ja-JP.ftl"), ja_content)
...
bytes.other = {count} Bytes
"#;
    std::fs::write(locale_dir.join("de-DE.ftl"), de_content)
        .expect("Failed to write de-DE.ftl");
...
"#;
        ...
        assert_eq!(translations.get("simple-key"), Some(&"Simple value".to_string()));
        assert_eq!(translations.get("multiline-key"), Some(&"This is a multiline value with continuation".to_string()));
        assert_eq!(translations.get("quoted-key"), Some(&"Quoted value with spaces".to_string()));
        assert_eq!(translations.get("key-with-placeholder"), Some(&"Hello {name}!".to_string()));
...
"#;
    ...
    fn test_thousands_separator() {
        ...
        assert_eq!(I18nManager::add_thousands_separator("1234567", ","), "1,234,567");
        assert_eq!(I18nManager::add_thousands_separator("1234567", " "), "1 234 567");
        assert_eq!(I18nManager::add_thousands_separator("1234567", "."), "1.234.567");
